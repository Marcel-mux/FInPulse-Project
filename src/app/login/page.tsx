...
function LoginForm() {
  ...
  // Error listener dari NextAuth query param
  useEffect(() => {
    const errorParam = searchParams.get("error");
    if (errorParam) {
      if (errorParam === "OAuthCallback" || errorParam === "OAuthSignin") {
        setLoginError("Gagal masuk dengan Google. Silakan coba lagi atau gunakan kata sandi.");
      } else {
        setLoginError(`Gagal autentikasi (${errorParam})`);
      }
    }
  }, [searchParams]);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.error("[AUTH-GOOGLE] Error signIn:", err);
      setLoginError(
        "Gagal mengalihkan ke Google. Silakan periksa koneksi atau coba lagi nanti."
      );
            ...
            {activeTab === "login" ? (
              ...
              >
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-charcoal-950 hover:bg-charcoal-800 border border-white/[0.08] text-gray-300 hover:text-white font-semibold text-xs sm:text-sm flex items-center justify-center gap-2.5 transition-all cursor-pointer disabled:opacity-50"
                >
                  {isGoogleLoading ? (
                    <>
